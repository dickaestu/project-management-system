...
class BoardController extends Controller
{
    public function index($id)
    {
        ...
        return view('pages.leader.board.index', compact('item', 'boards', 'logs'));
    }
    ...
    public function create(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $id,
            'activity' => "Project Leader" . ' has created '  . $request->board_name . ' board',
            'activity_icon' => '<i class="fas fa-square"></i>'
        ]);

        return redirect()->route('project-board-leader', $id)->with('success', 'Success Create' . $request->board_name);
    }
    ...
    public function editBoard(Request $request, $id)
    {
        ...
        LogActivity::create([
            ...
            'activity' => "Project Leader" . ' has renamed '  . $item->board_name . ' board into ' . $request->board_name . ' board',
    ...
    public function deleteBoard($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->projects_id,
            'activity' => 'Project Leader' . ' has deleted '  . '"' . $item->board_name . '"' . ' board',
    ...
    public function showTask($id)
    {
        ...
        return view('pages.leader.board.show', compact('item', 'boards'));
    }

    public function showCreateTask($id)
    {
        $id = $id;
        return view('pages.leader.board.create-task', compact('id'));
    }
    ...
    public function createTask(Request $request, $id)
    {
        ...
        LogActivity::create([
            ...
            'activity' => 'Project Leader' . ' has created '  . '"' . $item->task_name . '"' . ' task',
    ...
    public function archiveTask($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' archived '  . '"' . $item->task_name . '"' . ' task',
    ...
    public function deleteTask($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' has deleted '  . '"' . $item->task_name . '"' . ' task',
    ...
    public function taskNameEdit(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' has renamed task '  . '"' . $item->task_name . '"' . ' into ' . '"' . $request->task_name . '"' . ' task',
    ...
    public function uploadFileTask(Request $request, $id)
    {
        ...
        LogActivity::create([
            ...
            'activity' => 'Project Leader' . ' has uploaded new file named '  . '"' . $item->file_name . '"' . ' in ' . '"' . $item->board_task->task_name . '"' . ' task',
    ...
    public function deleteFileTask($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' has deleted '   . $item->file_name .  ' in ' . '"' . $item->board_task->task_name . '"' . ' task',
            'activity_icon' => '<i class="fas fa-trash"></i>'
        ]);
        return response()->json([
            'success' => 'Delete Successfully',
    ...
    public function storeSubTask(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' has created sub task '   . '"' . $item->sub_task_name . '"' . ' in ' . '"' . $item->board_task->task_name . '"' . ' task',
    ...
    public function changeStatusSubTask(Request $request, $id)
    {
        if ($request->status == 'true') {
            ...
            LogActivity::create([
                'projects_id' => $item->board_task->board->projects_id,
                'activity' => 'Project Leader' . ' has changed '   . '"' . $item->sub_task_name . '"' . ' sub task status from not completed into completed',
            ...
            LogActivity::create([
                'projects_id' => $item->board_task->board->projects_id,
                'activity' => 'Project Leader' . ' has changed '   . '"' . $item->sub_task_name . '"' . ' sub task status from completed into not completed',
    ...
    public function deleteSubTask($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' has deleted sub task '   . '"' . $item->sub_task_name . '"' . ' in ' . '"' . $item->board_task->task_name . '"' . ' task',
            'activity_icon' => '<i class="fas fa-trash"></i>'
        ]);
        return response()->json([
            'success' => 'Delete Successfully',
    ...
    public function changeTags(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' has changed tags '   . '"' . $item->tags . '"' . ' to ' . '"' . $request->tags . '"' . ' in ' . '"' . $item->task_name . '"' . ' task',
            'activity_icon' => '<i class="fas fa-pencil-alt"></i>'
        ]);
        $data = $request->all();
        $item->update($data);

        return redirect()->route('project-board-leader', $item->board->projects_id);
    }
    ...
    public function addComment(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' has commented on '  . $item->board_task->task_name . ' task',
    ...
    public function changeStatus(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' has changed '  . $item->task_name . ' task from ' . $task->board->board_name . ' board into ' . $item->board->board_name . ' board',
    ...
    public function createTaskMember(Request $request)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' assigned '  . $item->project_members->user->name . ' into ' . $item->board_task->task_name . ' task',
    ...
    public function deleteTaskMember($id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board_task->board->projects_id,
            'activity' => 'Project Leader' . ' has removed '  . $item->project_members->user->name . ' in ' . $item->board_task->task_name . ' task',
    ...
    public function taskDescriptionUpdate(Request $request, $id)
    {
        ...
        LogActivity::create([
            'projects_id' => $item->board->projects_id,
            'activity' => 'Project Leader' . ' has updated the description of '  . $item->task_name . ' task',
            'activity_icon' => '<i class="fas fa-pencil-alt"></i>'
        ]);
