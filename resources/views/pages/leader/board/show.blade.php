...
<div class="row">
    <div class="col-12 col-lg-7 mb-4 mb-lg-0">
        ...
        <form action="{{ route('change-tags-leader', $item->id) }}" class="form-tags mt-3" method="post">
            @csrf
            @method('PUT')
            <div class="form-group mb-2">
            ...
            <ul  id="list-group-sub-task" class="list-group">
                ...
                <li class="list-group-item list-sub-task d-flex justify-content-between">
                    ...
                    <span class="d-block">
                        <a
                        href="#"
                        data-url="{{ route('change-status-sub-task-leader', $subTask->id) }}?status=true"
                    ...
                    <a
                    href="#"
                    data-url="{{ route('change-status-sub-task-leader', $subTask->id) }}?status=false"
                ...
                data-url="{{ route('delete-sub-task-leader', $subTask->id) }}"
        ...
        <div class="card-body">
            <ul class="list-group">
                ...
                <li class="list-group-item list-file d-md-flex  justify-content-between align-items-center">
                    {{ $file->file_name }}
                    <span class="d-block">
                        <a href="{{ route('download-file-task-leader', $file->id) }}" class="btn btn-primary btn-sm"><i class="fas fa-download"></i></a>
                        
                        <button type="button" 
                        data-token = "{{ csrf_token() }}"
                        data-url="{{ route('delete-file-task-leader', $file->id) }}"
...
<div class="form-group">
    ...
    <div class="activity float-left">
        ...
        <button
        data-token = "{{ csrf_token() }}"
        data-url="{{ route('delete-comment-leader', $comment->id) }}"
...
<div class="col-12 col-md-5">
    <form action="{{ route('change-status-leader',$item->id) }}" class="mb-3" method="POST">
        @method('PUT')
        @csrf
    ...
    <div class="row task-members pl-0 mb-3">
        ...
        <div class="col-2 pt-2">
            ...
            data-id = "{{ $task->id }}"
            data-token = "{{ csrf_token() }}"
            data-url="{{ route('delete-task-member-leader', $task->id) }}"
...
<script>
    $('document').ready( function(){
        ...
        $('.form-upload-file').on('submit', function(e){
            ...
            $.ajax({
                url: '{{ route('upload-file-task-leader', $item->id) }}',
                type: 'POST',
                data: data,
                processData: false,
                contentType: false,
                ...
                success: function(response){
                    ...
                    <li class="list-group-item list-file d-md-flex  justify-content-between align-items-center">
                        `+response.name+`
                        <span class="d-block">
                            <a href="/leader/download-task-file/`+response.id+`" class="btn btn-primary btn-sm"><i class="fas fa-download"></i></a>
                            <form class="d-inline" action="" method="post">
                                <button type="button" 
                                data-name="`+response.name+`"
                                data-token = "{{ csrf_token() }}"
                                data-url="/leader/delete-task-file/`+response.id+`"
                                class="btn btn-danger btn-sm button-delete"><i class="fas fa-trash"></i></button>
                            </form>
                        </span>
                    </li>
        ...
        $('.form-sub-task').on('submit', function(e){
            ...
            $.ajax({
                url: '{{ route('add-sub-task-leader', $item->id) }}',
                type: 'POST',
                data: data,
                dataType: 'json',
                success: function(response){
                    ...
                    <li class="list-group-item list-sub-task d-flex justify-content-between">`+response.sub_task_name+`
                        <span class="d-block">
                            <a
                            href="#"
                            data-url="/leader/status-sub-task/`+response.id+`?status=true"
                            class="btn rounded-circle btn-sm py-0 btn-outline-success btn-sub-task-completed">
                            <i class="fas fa-check"></i>
                        </a>
                        <button 
                        type="button"
                        data-token = "{{ csrf_token() }}"
                        data-url="/leader/delete-sub-task/`+response.id+`"
    ...
    $('.form-update-description').on('submit', function(e){
        ...
        var data = $this.serializeArray();  
        $.ajax({
            url: '{{ route('task-description-update-leader', $item->id) }}',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function(response){
    ...
    $('#project_members_id').select2({
        ...
        ajax: {
            url :'{{ route('cari-member-leader', $item->board->projects_id) }}',
            dataType : 'json',
            delay: 250,
    ...
    $('.form-assign').on('submit', function(e){
        ...
        $.ajax({
            url: '{{ route('create-task-member-leader') }}',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function(response){
                if(response.failed == 'error'){
                    ...
                    <div class="row task-members pl-0 mb-3">
                        ...
                        <div class="col-2 pt-2">
                            ...
                            data-id = "`+response.id +`"
                            data-token = "{{ csrf_token() }}"
                            data-url="/leader/delete-task-member/`+response.id +`"
                            data-name="`+ response.name+`"
                            class="btn button_delete rounded-circle btn-sm py-0 btn-outline-danger"
                            >
                            <i class="fas fa-minus"></i>
...
$('.form-comment').on('submit', function(e){
    ...
    var data = $this.serializeArray(); 
    $.ajax({
        url: '{{ route('add-comment-leader', $item->id) }}',
        type: 'POST',
        data: data,
        dataType: 'json',
        success: function(response){
...
$('.form-edit-task-name').on('submit', function(e){
    ...
    var data = $this.serializeArray();  
    $.ajax({
        url: '{{ route('edit-task-name-leader', $item->id) }}',
        type: 'POST',
        data: data,
        dataType: 'json',
        success: function(response){
...
$("#list-group-sub-task").on('click','.btn-sub-task-completed', function (event) {
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            <a
            href="#"
            data-url="/leader/status-sub-task/${response.id}?status=false"
            class="text-decoration-none text-black-50 btn-sub-task-uncompleted">
